...
import { FileSystemWriter, ReporterRuntime, getEnvironmentLabels, getSuiteLabels, parseTestPlan } from "allure-js-commons/sdk/reporter";
...
export class AllureCypress {
  ...
  attachToCypress(on) {
    on("task", {
      readAllureTestPlan: () => {
        var _parseTestPlan;
        return (_parseTestPlan = parseTestPlan()) !== null && _parseTestPlan !== void 0 ? _parseTestPlan : null;
      },
  ...
  endSpec(spec, cypressVideoPath) {
    ...
    var isSpecFailed = specMessages.some(message => message.type === "cypress_test_end" && (message.data.status === Status.FAILED || message.data.status === Status.BROKEN));
    ...
    specMessages.forEach((message, i) => {
      ...
      var lastHookMessage = toReversed(previousMessagesSlice).find(_ref3 => {
        var {
          type
        } = _ref3;
      ...
      if (message.type === "cypress_hook_start") {
        var fixtureUuid = this.allureRuntime.startFixture(last(runContext.scopes), message.data.type, {
      ...
      if (message.type === "cypress_test_start") {
        var suiteLabels = getSuiteLabels(message.data.specPath.slice(0, -1));
        ...
        var testUuid = this.allureRuntime.startTest({
          ...
          labels: [{
            ...
            value: "cypress"
          }, ...suiteLabels, ...titleMetadata.labels, ...getEnvironmentLabels()]
        }, runContext.scopes);
      ...
      if (message.type === "cypress_command_start") {
        var lastExecutableUuid = last(runContext.executables);
        var lastStepUuid = last(runContext.steps);
      ...
      this.allureRuntime.applyRuntimeMessages(last(runContext.executables), [message]);
    });
